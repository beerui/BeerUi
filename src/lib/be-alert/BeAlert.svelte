...
>
  ...
  <div class="be-alert__content">
    ...
    {#if closable}
	    {#if closeText}
		    <i role='button' tabindex='-1' class='be-alert__closebtn is-customed' on:click={close} on:keydown>{closeText}</i>
	    {:else}
		    <i role='button' tabindex='-1' class='be-alert__closebtn be-icon be-icon-close' on:click={close} on:keydown></i>
	    {/if}
    {/if}
  </div>
  </div>
