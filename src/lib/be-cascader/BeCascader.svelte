<script lang='ts'>
	...
	$: if ((value && value.length) || !value) {
		inputValue = []
		initInputValue(options);
		if (cascaderStore) cascaderStore.value = value
	}
...
<div
	...
	on:keyup
>
	<div on:mouseover={() => {if(clearable && inputValue) showClose = true}}
	...
	<CascaderPanel bind:this={Panel} {visible} {options} {expandTrigger} bind:defaultValue={value} on:change={change} />
</div>
