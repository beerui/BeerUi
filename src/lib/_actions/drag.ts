 ...
 * 注册元素的拖拽事件
 * @param dragBox
 * @param options { limit：false 限制在可视范围内, isDrag: true 可拖拽 }
 * @constructor
 */
export default function DragEvent(dragBox: HTMLElement, options: DragElement = { isDrag: true, limit: true }) {
	if (!options.isDrag) return
	const target = dragBox;
	const dragArea = dragBox.querySelector('.dragArea');
	const clickHandler = (targetEvent: MouseEvent) => {
		// 算出鼠标相对元素的位置
	...
	on(dragArea, 'mousedown', clickHandler)
	return {
		destroy: () => {
